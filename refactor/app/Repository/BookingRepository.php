...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $jobs = $cuser->translatorJobs()->where('status', 'new')->get();
            $usertype = 'translator';
        }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $page = $request->get('page', 1); // Use default value 1 if page is not set
     ...
     * @return mixed
     */
    public function store(BookingRequest $request, $user, $data)
    {
        if ($user->user_type == config('constants.customer_role_id')) {
            $cuser = $user;

            // Validate the request using the BookingRequest validation rules

            // Handle immediate booking
            if ($data['immediate'] === 'yes') {
                $due = now()->addMinutes(config('constants.immediate_time'));
            } else {
            ...
            // Map 'customer_phone_type' and 'customer_physical_type' to 'yes' or 'no'

            // Map 'job_for' values

            // Map 'job_type' based on 'consumer_type'

            $data['due'] = $due->format('Y-m-d H:i:s');
            $data['b_created_at'] = now();
            $data['will_expire_at'] = TeHelper::willExpireAt($due, $data['b_created_at']);
            $data['by_admin'] = $request->input('by_admin', 'no');
     ...
     * @return mixed
     */
    public function storeJobEmail($data, Job $job)
    {
        ...
        if (isset($data['address'])) {
            // Use conditional ternaries for default values
    ...
    public function jobToData($job)
    {
        ...
        $jobForMap = [
            ...
            'law' => 'Rätttstolk',
            'n_law' => 'Rätttstolk', // Assuming 'n_law' is also 'Rätttstolk'
        ];

        $jobFor = $jobForMap[$job->gender] ?? null;
        $certified = $jobForMap[$job->certified] ?? null;

        if ($jobFor !== null) {
            $data['job_for'] = is_array($jobFor) ? $jobFor : [$jobFor];
        }

        if ($certified !== null && $certified !== $jobFor) {
    ...
    public function jobEnd($post_data = array(), AppMailer $mailer)
    {
        ...
        // Calculate the session time using Carbon
        $start = Carbon::parse($job->due);
        $end = $completedDate;
        $interval = $end->diffAsCarbonInterval($start)->format('%h:%I:%S');
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $filteredJobIds = array_filter($jobIds, function ($jobId) use ($user_id) {
